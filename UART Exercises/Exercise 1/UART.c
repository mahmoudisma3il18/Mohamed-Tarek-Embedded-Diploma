...
uint8_t* UART_recieveString(void) {
	...
	uint8_t *a_recived_data ; /* Pointer to chars to hold received data */
	
	*a_recived_data = UART_recieveByte(); /* Receive one byte  */
	
	while (*a_recived_data != '\0')
	{
		a_recived_data++; /* Increment Pointer */
		
		*a_recived_data = UART_recieveByte(); 
	}
	
	return a_recived_data; /* Return Pointer to char  */
}
